...
const UrunKremQuiz = () => {
  ...
  const getRecommendation = () => {
    // KURAL 1: Açık yara veya kronik hastalık → Doktora yönlendir
    if (answers.q3 === 'evet' || answers.q4 === 'evet') {
      return {
        title: "⚠️ Öncelikle Doktorunuza Danışın!",
        type: "warning",
        icon: "🩺",
        description: "Açık yaralar veya diyabet gibi kronik rahatsızlıklar, ayak bakımında profesyonel bir yaklaşım gerektirir.",
        product: "",
        reason: "Herhangi bir ürün kullanmadan önce mutlaka doktorunuza veya podoloğunuza danışarak onay almalısınız. Sağlığınız bizim için en önemli önceliktir. Onay sonrası ürünlerimiz tedavinize destek olabilir.",
        usage: "",
        showCTA: false
      };
    }

    // KURAL 2: Mantar belirtileri var → 3'lü Set öner
    if (answers.q2 === 'evet') {
      return {
        title: "Komple Bakım Seti Sizin İçin İdeal!",
        type: "set",
        icon: "🎁",
        description: "Cevaplarınız, cildinizin hem yoğun neme ihtiyaç duyduğunu hem de mantar belirtileri gösterdiğini işaret ediyor. Bu durumda tek bir ürün yeterli olmayabilir.",
        product: "Pedizone 3'lü Bakım Seti: Krem + Serum + Köpük",
        reason: "En etkili sonuç için 3'lü Bakım Seti kullanmanızı öneriyoruz:\n\n• Antifungal Serum: Enfeksiyon belirtilerini (kaşıntı, koku, soyulma) doğrudan hedefler.\n• Intense Repair Krem: Cildin kaybettiği nemi geri kazandırır, çatlakları onarır ve antifungal içeriğiyle tedaviyi destekler.\n• Temizleme Köpüğü: Günlük hijyeni sağlayarak sorunun tekrarlamasını önlemeye yardımcı olur.",
        usage: "Bu kombinasyon, sorunu her açıdan ele alarak size tam bir çözüm sunar.",
        showCTA: true
      };
    }

    // KURAL 3: Sadece kuruluk → Krem uygun
    return {
      ...
      usage: "Düzenli kullanımda ayaklarınızın daha pürüzsüz ve sağlıklı olduğunu göreceksiniz.",
      showCTA: true
    };
  };
